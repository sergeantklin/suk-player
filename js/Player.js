function SUK_Player(_initParams) {
  var initParams = _initParams || {},
    ...
    BUFFER_SIZE = 2048;
  var useHtmlPlayer =
    !audioContext || navigator.userAgent.toLowerCase().indexOf("webkit") == -1;
  ...
  if (useHtmlPlayer) {
    ...
    htmlPlayer.ontimeupdate = function () {
      initParams.onTimeUpdate &&
        initParams.onTimeUpdate(htmlPlayer.currentTime);
    };
  ...
  function attachPitchPlayer() {
    ...
    const chan = soundBuffer.numberOfChannels;
    audioNode = audioContext.createScriptProcessor(4096, chan, chan);

    // audioNode.buffer = soundBuffer;
    // audioNode.playbackRate.value = tempo;
  ...
  function getCurrentTime() {
    // console.log(pausedAt, startedAt, audioContext.currentTime);
    if (pausedAt) {
      return pausedAt;
    }
    if (startedAt || playing) {
  ...
  function play(supress) {
    ...
    clearInterval(playInterval);
    playInterval = setInterval(onPlay, 100);
  ...
  function getBlobs(_url) {
    ...
    return {
      ...
      audio: recorder.getOriginalBuffer().length
        ? new Blob([audioBufferToWav(recorder.getOriginalBuffer())], {
            type: "audio/webm",
          })
        : null,
